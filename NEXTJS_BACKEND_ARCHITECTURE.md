...
## BizLens Analytical Engine — May 12, 2026

The BizLens engine has been fully ported into the `lib/services/bizlens-service.ts` layer. 

### The Compute Pattern

To ensure high performance and UI stability, all analytical metrics are pre-calculated on the server before being passed to the client-side 'Bento' components.

1. **Data Ingestion**: Facts are fetched from the `bizlens_data` table.
2. **Service Computation**: `computeReport(facts)` generates a comprehensive report object containing:
   - **KPIs**: Revenue, EBITDA, Margin, Burn Rate.
   - **Health Diagnostics**: Liquidity, Discipline, Concentration metrics.
   - **Working Capital**: WCC days, AR/AP aging impact.
   - **Financial Freedom**: Debt freedom date, Break-even days.
3. **Insight Generation**: `generateInsights(report)` produces categorized opportunities and warnings.
4. **Dashboard Delivery**: The `BizlensOutputDashboard` receives the processed `report` and `insights` as props, eliminating the need for complex math on the client.

### Type Safety

The engine uses the `BizlensData` interface (defined in `types/bizlens.ts`) which enforces the presence of all 45+ financial inputs required for a valid diagnostic.

### Boundary Enforcement

Heavy analytical functions are kept in the service layer to prevent the 'use client' boundary from pulling in server-side dependencies. Client-side interaction (like the Scenario Simulator) uses a lightweight, pure-math variant of the logic to ensure instant feedback.

---

## Summary of Truth

Follow the **Action → Service → Repository** pattern for all new features. Ensure the build passes `next build` after every major functional change to catch type regressions early.
